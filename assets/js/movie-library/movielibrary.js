...
// MUDANÇA: Atualizada a função toggleMovieDetails
function toggleMovieDetails(index) {
  const movie = movies[index];
  console.log("Toggling details for movie:", movie);
...
// MUDANÇA: Atualizado o listener do formulário de edição
if (editForm) {
  editForm.addEventListener("submit", function (e) {
    e.preventDefault();
    ...
    // Atualizar o card e os detalhes imediatamente
    updateMovieCard(currentEditIndex);
    updateMovieDetails(currentEditIndex);
...
// NOVA FUNÇÃO: Adicionar esta função para atualizar o card do filme
function updateMovieCard(index) {
  const movieCard = document.querySelector(
    `.movie-card[data-index="${index}"]`
  );
...
document.addEventListener("DOMContentLoaded", () => {
  console.log("Page loaded, initial movies:", movies);
  renderMovies();
});
